...
document.addEventListener("click",event=>{if(document.body.classList.contains("inline-editor-mode")&&event.target.closest("#editor-grid [data-pp-trigger],#editor-grid [data-pp-playlist-trigger],#editor-grid [data-pp-nav],#editor-grid [data-pp-item-nav],#editor-grid [data-pp-macro],#editor-grid [data-proclaim-trigger],#editor-grid [data-proclaim-command],#editor-grid [data-board-destination],#editor-grid [data-service-action],#editor-grid [data-lighting-button],#editor-grid [data-mixer-mute]")){event.preventDefault();event.stopImmediatePropagation()}},true);
...
document.addEventListener("click",async event=>{const button=event.target.closest("[data-proclaim-command]");if(!button||button.disabled)return;const status=button.closest(".proclaim-control-pad")?.querySelector("[data-proclaim-status]");button.disabled=true;if(status)status.textContent="Sending command to Proclaim…";try{await api(`/api/integrations/proclaim/command/${button.dataset.proclaimCommand}`,{method:"POST",body:JSON.stringify(triggerScope(button))});if(status)status.textContent="Proclaim command sent";await refresh(true)}catch(error){if(status)status.textContent=error.message}finally{button.disabled=false}});
document.addEventListener("click",async event=>{const button=event.target.closest("[data-proclaim-trigger]");if(!button||button.disabled)return;const itemIndex=Number(button.dataset.proclaimItemIndex),slideIndex=Number(button.dataset.proclaimSlideIndex);if(!Number.isInteger(itemIndex)||itemIndex<0||!Number.isInteger(slideIndex)||slideIndex<0)return;button.disabled=true;try{await api("/api/integrations/proclaim/trigger",{method:"POST",body:JSON.stringify({item_index:itemIndex,slide_index:slideIndex,...triggerScope(button)})});await refresh(true)}catch(error){alert(error.message)}finally{button.disabled=false}});
...
function widgetStateKey(widget,state){
  ...
  if(widget.type==="proclaim_controls"){const proclaim=state.proclaim||{};return`proclaim-controls:${!!proclaim.connected}:${!!proclaim.on_air}:${String(proclaim.error||"")}`}
  if(widget.type==="proclaim_slides"){const proclaim=state.proclaim||{},current=proclaim.current||{},next=proclaim.next||{};return`proclaim-slides:${JSON.stringify([proclaim.connected,proclaim.on_air,current.id,current.slide_index,current.text,next.id,next.slide_index,next.text,settings])}`}
  if(widget.type==="proclaim_playlist"){const proclaim=state.proclaim||{};return`proclaim-playlist:${JSON.stringify([proclaim.current,proclaim.playlist_items||[],settings])}`}
  if(widget.type==="proclaim_timers"){const proclaim=state.proclaim||{};return`proclaim-timers:${JSON.stringify(proclaim.timers||[])}`}
