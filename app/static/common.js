...
const micCardMarkup = (mic,mode="photos",options={}) => {
  const person=mic.assignment||{},health=micHealth(mic),active=micIsActive(mic),displayName=person.name&&person.name!=="Position unfilled"?person.name:"Unassigned",position=[person.team_name,person.position].filter(Boolean).join(" · ")||"Unmapped position",numericChannel=Number(mic.channel),channel=Number.isFinite(numericChannel)?String(numericChannel).padStart(2,"0"):"—",status=mic.manual?"NOT NETWORKED":mic.pack?(mic.online?(mic.muted?"MUTED":"LIVE"):"OFFLINE"):mic.placeholder?"NO MIC":active?(mic.muted?"MUTED":`${mic.battery_percent||0}% BAT`):"OFFLINE",equipment=mic.equipment||[mic];
  const itemLabel=item=>[item.name||item.receiver,Number(item.channel)>0&&!item.pack?`Ch ${item.channel}`:""].filter(Boolean).join(" · ");
  ...
  const statsBlock=mic.manual?`<div class="technical-nonetworked">No telemetry — not a networked receiver</div>`:mic.pack?`<div class="technical-stats"><div><span>Audio</span><strong>${mic.audio||0}%</strong></div><div><span>Status</span><strong>${mic.muted?"Muted":"Live"}</strong></div></div>`:`<div class="technical-stats">${[["Battery",mic.battery_percent],["RF",mic.rf],["Audio",mic.audio]].map(([label,value])=>`<div><span>${label}</span><strong>${value||0}%</strong></div>`).join("")}</div>`;
  if(mode==="technical")return `<article class="mic-card technical-tile ${health} ${mic.manual?"manual":""} ${mic.pack?"pack":""} ${mic.placeholder?"no-mic":""}"><div class="technical-header"><div><strong>${escapeHtml(displayName)}</strong><span>${escapeHtml(position)}</span></div><b>${status}</b></div><div class="technical-gear">${gearMarkup||`<span>${escapeHtml(hardware||"—")}</span>`}</div>${statsBlock}${[mic.frequency,mic.tx_type,mic.antenna,mic.diversity,mic.rf_power].filter(Boolean).length?`<div class="technical-meta">${[mic.frequency,mic.tx_type,mic.antenna,mic.diversity,mic.rf_power].filter(Boolean).map(escapeHtml).join(" · ")}</div>`:""}${!mic.manual&&mic.errors?.length?`<div class="technical-error">${escapeHtml(mic.errors[0])}</div>`:""}</article>`;
