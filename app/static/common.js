...
const micIsActive = mic => mic.pack?mic.online===true:mic.online===true&&Number.isFinite(Number(mic.battery_percent));
const micHealth = mic => mic.manual?"manual":mic.pack?(mic.online===true?(mic.muted?"low":"healthy"):"critical"):!micIsActive(mic)||Number(mic.battery_percent)<5?"critical":Number(mic.battery_percent)<=10?"low":"healthy";
const micMeterRow = ([label,value]) => `<div><div class="meter-label"><span>${label}</span><span>${value||0}%</span></div><div class="meter-track"><div class="meter-fill" style="width:${value||0}%"></div></div></div>`;
const micMeters = mic => `<div class="meters">${(mic.pack?[["AUD",mic.audio]]:[["BAT",mic.battery_percent],["RF",mic.rf],["AUD",mic.audio]]).map(micMeterRow).join("")}</div>`;
...
const stagePlotMarkerInner = (entry,showRf) => {
  ...
  const firstName=String(entry.name||"").trim().split(/\s+/)[0]||entry.name||"";
  return `<span class="sp-photo">${photo}${rf}</span><span class="sp-name" title="${escapeHtml(entry.name)}">${escapeHtml(firstName)}</span>${entry.mic?`<span class="sp-mic">${escapeHtml(entry.mic)}</span>`:""}`;
};
...
const proclaimPlaylistMarkup = (proclaim,allowTrigger=false,options={}) => {
  const density=["compact","comfortable","large","extra_large","huge","maximum"].includes(options.density)?options.density:"comfortable",sizes={compact:[88,40,9],comfortable:[120,48,10],large:[170,60,12],extra_large:[280,72,14],huge:[420,84,16],maximum:[640,96,18]},[slideSize,itemSize,markerSize]=sizes[density],activeBorderColor=safeCssColor(options.active_border_color||"#f5c400"),items=Array.isArray(proclaim.playlist_items)?proclaim.playlist_items:[];
  if(!items.length)return `<div class="pp-browser-empty"><div class="empty">${escapeHtml(proclaim.connected===false?proclaim.error||"Proclaim is disconnected":"Put a presentation on air in Proclaim to see its service items")}</div></div>`;
  const currentItem=Number(proclaim.current?.item_index),currentSlide=Number(proclaim.current?.slide_index);
  const rows=items.map(item=>{const itemIndex=Number(item.index),active=itemIndex===currentItem||item.active,slides=(item.slides||[]).map(slide=>{const slideIndex=Number(slide.slide_index),slideNumber=Number(slide.index)||slideIndex+1,isActive=active&&slideIndex===currentSlide,text=String(slide.text||`Slide ${slideNumber}`);return `<button type="button" class="pp-list-slide text-mode ${isActive?"active":""}" data-proclaim-fallback-text="${escapeHtml(text)}" ${allowTrigger&&item.triggerable!==false?`data-proclaim-trigger data-proclaim-item-index="${itemIndex}" data-proclaim-slide-index="${slideIndex}"`:"disabled"} title="Trigger ${escapeHtml(item.title)} slide ${slideNumber}"><span class="pp-slide-number">${slideNumber}</span><span class="pp-slide-fallback pp-slide-text">${escapeHtml(text)}</span></button>`}).join("");if(item.triggerable===false)return `<div class="pp-list-placeholder"><span>${itemIndex+1}</span><div><strong>${escapeHtml(item.title)}</strong><small>Service section</small></div></div>`;return `<section class="pp-list-presentation ${active?"active":""}" data-proclaim-item-index="${itemIndex}"><button type="button" class="pp-list-presentation-header" ${allowTrigger?`data-proclaim-trigger data-proclaim-item-index="${itemIndex}" data-proclaim-slide-index="0"`:"disabled"}><span>${itemIndex+1}</span><strong>${escapeHtml(item.title)}</strong><small>${active?"On air":`${(item.slides||[]).length} slides`}</small></button>${slides?`<div class="pp-list-slides">${slides}</div>`:`<div class="pp-list-slides-empty">No slide text returned by Proclaim</div>`}</section>`}).join("");
  return `<div class="pp-browser proclaim-browser" data-proclaim-auto-scroll="${options.auto_scroll!==false}" style="--pp-slide-size:${slideSize}px;--pp-item-size:${itemSize}px;--pp-marker-size:${markerSize}px;--pp-active-border:${activeBorderColor}"><header><div><strong>${escapeHtml(proclaim.presentation_name||"Proclaim presentation")}</strong><span>${items.length} items</span></div><small>${escapeHtml(proclaim.current?.title||"No active item")}</small></header><div class="pp-browser-scroll">${rows}</div></div>`;
};
...
const micCardMarkup = (mic,mode="photos",options={}) => {
  const person=mic.assignment||{},health=micHealth(mic),active=micIsActive(mic),displayName=person.name&&person.name!=="Position unfilled"?person.name:"Unassigned",position=[person.team_name,person.position].filter(Boolean).join(" · ")||"Unmapped position",numericChannel=Number(mic.channel),channel=Number.isFinite(numericChannel)?String(numericChannel).padStart(2,"0"):"—",status=mic.manual?"":mic.pack?(mic.online?(mic.muted?"MUTED":"LIVE"):"OFFLINE"):mic.placeholder?"NO MIC":active?(mic.muted?"MUTED":`${mic.battery_percent||0}% BAT`):"OFFLINE",equipment=mic.equipment||[mic];
  const itemLabel=item=>[item.name||item.receiver,(!item.name&&Number(item.channel)>0&&!item.pack)?`Ch ${item.channel}`:""].filter(Boolean).join(" · ");
  ...
  const gearLines=(mic.placeholder||person.standalone_mic)?[]:equipment.map(item=>({label:slotName[item.slot]||"",text:itemLabel(item)+(item.pack&&item.muted?" · MUTED":"")})).filter(line=>line.text);
  ...
  const statsBlock=mic.manual?`<div class="technical-nonetworked">No telemetry</div>`:mic.pack?`<div class="technical-stats"><div><span>Audio</span><strong>${mic.audio||0}%</strong></div><div><span>Status</span><strong>${mic.muted?"Muted":"Live"}</strong></div></div>`:`<div class="technical-stats">${[["Battery",mic.battery_percent],["RF",mic.rf],["Audio",mic.audio]].map(([label,value])=>`<div><span>${label}</span><strong>${value||0}%</strong></div>`).join("")}</div>`;
  if(mode==="technical")return `<article class="mic-card technical-tile ${health} ${mic.manual?"manual":""} ${mic.pack?"pack":""} ${mic.placeholder?"no-mic":""}"><div class="technical-header"><div><strong>${escapeHtml(displayName)}</strong><span>${escapeHtml(position)}</span></div>${status?`<b>${status}</b>`:""}</div><div class="technical-gear">${gearMarkup||`<span>${escapeHtml(hardware||"—")}</span>`}</div>${statsBlock}${[mic.frequency,mic.tx_type,mic.antenna,mic.diversity,mic.rf_power].filter(Boolean).length?`<div class="technical-meta">${[mic.frequency,mic.tx_type,mic.antenna,mic.diversity,mic.rf_power].filter(Boolean).map(escapeHtml).join(" · ")}</div>`:""}${!mic.manual&&mic.errors?.length?`<div class="technical-error">${escapeHtml(mic.errors[0])}</div>`:""}</article>`;
  ...
  return `<article class="mic-card talent-tile ${health} ${person.photo?"has-photo":""} ${mic.manual?"manual":""} ${mic.pack?"pack":""} ${mic.placeholder?"no-mic":""}"><div class="talent-media">${portrait}</div><div class="talent-gradient"></div><div class="talent-identity"><div class="mic-person">${escapeHtml(displayName)}</div>${gearMarkup?`<div class="mic-gear">${gearMarkup}</div>`:""}<div class="mic-position">${escapeHtml(position)}</div><div class="mic-hardware">${escapeHtml(gearLines.length?status:[hardware,status].filter(Boolean).join(" · "))}</div></div>${mic.manual?`<div class="mic-nonetworked">No telemetry</div>`:micMeters(mic)}</article>`;
};
...
let producerQrPromise;
const hydrateProducerQr = root => {
  const targets=[...root.querySelectorAll("[data-producer-qr-url]")];if(!targets.length)return;
  producerQrPromise??=api("/api/producer/qr-info").catch(()=>({url:"/producer"}));
  producerQrPromise.then(result=>targets.forEach(target=>{target.href=result.url||"/producer";target.textContent=result.url||"Open Producer"}));
};
...
const enhanceDynamicContent = (root=document) => {
  ...
  hydrateBoardNavigation(root);hydrateProducerQr(root);requestAnimationFrame(()=>root.querySelectorAll("[data-fit-slide]").forEach(element=>{
...
const widgetNames = {clock:"Clock",board_navigation:"Board navigation",service:"Service",timing:"Timers",assignments:"Scheduled Positions & Mics",mics:"Scheduled Positions & Mics",producer_qr:"Producer QR code",proclaim_slides:"Proclaim slides",proclaim_playlist:"Proclaim playlist",proclaim_controls:"Proclaim controls",proclaim_timers:"Proclaim timers",slides:"ProPresenter slides",playlist:"ProPresenter playlist",pp_controls:"ProPresenter controls",pp_macros:"ProPresenter macros",notes:"Slide notes",sermon_notes:"Sermon notes",order:"Order of service",people:"Team members",spl:"Open Sound Meter",prodmesh_rta:"ProdMesh RTA",behringer_faders:"Behringer faders",lighting:"Lighting controls",controls:"Service controls",person:"Scheduled person",restream:"Restream livestream",livestreams:"Livestream status",obs:"OBS live monitor",ndi:"NDI® video",webcam:"USB camera",propresenter_timers:"ProPresenter timers",text:"Custom text",rich_text:"Notes"};
const RICH_TEXT_ALLOWED_TAGS = new Set(["B","STRONG","I","EM","U","UL","OL","LI","BR","P","DIV","SPAN"]);
...
const legacyWidgetMarkup = (widget, state) => {
  const settings=widget.settings||{}, service=state.service||{}, timing=state.timing||{}, pp=state.propresenter||{}, proclaim=state.proclaim||{};
  let content="";
  if(widget.type==="playlist") content=propresenterPlaylistMarkup(pp,settings.allow_remote_trigger!==false,settings);
  if(widget.type==="proclaim_playlist") content=proclaimPlaylistMarkup(proclaim,settings.allow_remote_trigger!==false,settings);
  ...
  if(widget.type==="proclaim_controls") content=`<div class="pp-control-pad proclaim-control-pad"><button type="button" data-proclaim-command="previous-slide"><span>◀</span>Previous slide</button><button type="button" data-proclaim-command="next-slide"><span>▶</span>Next slide</button><button type="button" data-proclaim-command="previous-item"><span>◀◀</span>Previous item</button><button type="button" data-proclaim-command="next-item"><span>▶▶</span>Next item</button><button type="button" data-proclaim-command="on-air">Go on air</button><button type="button" data-proclaim-command="off-air">Go off air</button><div data-proclaim-status>${escapeHtml(proclaim.connected?(proclaim.on_air?"Proclaim · On air":"Proclaim · Connected"):proclaim.error||"Proclaim offline")}</div></div>`;
  if(widget.type==="proclaim_slides"){const current=proclaim.current||{},next=proclaim.next||{},source=String(proclaim.ndi_source_name||""),slideCount=Number(current.total_slides||0),slideNumber=Number(current.slide_number||0),slideLabel=slideCount?`<em>Slide ${slideNumber} of ${slideCount}</em>`:"",nextCount=Number(next.total_slides||0),nextNumber=Number(next.slide_number||0),nextLabel=nextCount?`<em>Slide ${nextNumber} of ${nextCount}</em>`:"";if(settings.preview_mode==="ndi"&&source){content=`<div class="proclaim-ndi"><img data-ndi-preview data-ndi-source="${escapeHtml(source)}" src="/api/integrations/ndi/snapshot?source=${encodeURIComponent(source)}" alt="Proclaim output"><span>${escapeHtml(current.title||"Current Proclaim item")} ${slideLabel}</span></div>`}else{const cards=[];if(settings.show_current!==false)cards.push(`<div class="proclaim-text-card current"><small>NOW</small><span class="proclaim-item-title">${escapeHtml(current.title||"Current item")}</span><strong data-fit-widget-text>${escapeHtml(current.text||current.title||"No current slide")}</strong>${slideLabel}</div>`);if(settings.show_next!==false)cards.push(`<div class="proclaim-text-card"><small>NEXT</small><span class="proclaim-item-title">${escapeHtml(next.same_item?"":next.title||"")}</span><strong data-fit-widget-text>${escapeHtml(next.text||next.title||"No next slide")}</strong>${nextLabel}</div>`);content=`<div class="proclaim-text-slides ${cards.length===1?"one":""}">${cards.join("")}</div>`}}
  ...
  if(widget.type==="producer_qr") content=`<div class="producer-qr" data-producer-qr><div class="producer-qr-code"><img src="/api/producer/qr-code" alt="QR code to open ChurchBoard Producer"></div><strong>Scan to open Producer</strong><a data-producer-qr-url href="/producer">Finding this ChurchBoard address…</a><small>Connect your phone to the same network</small></div>`;
  ...
  if(widget.type==="proclaim_timers") { const timers=Array.isArray(proclaim.timers)?proclaim.timers:[];content=timers.length?`<div class="pp-timers">${timers.map(timer=>`<div class="pp-timer ${escapeHtml(timer.state||"stopped")}"><span>${escapeHtml(timer.name||"Proclaim timer")}</span><strong data-fit-widget-text>${escapeHtml(timer.time||"—")}</strong><small>${escapeHtml(timer.state||"stopped")}</small></div>`).join("")}</div>`:`<div class="empty">No active Proclaim countdown or target timer</div>`; }
  ...
  if(widget.type==="rich_text") content=`<div class="rich-text-notes" data-rich-text-scroll tabindex="0">${sanitizeRichText(settings.html||"")||'<p class="empty">No content yet — edit this widget to add notes</p>'}</div>`;
